...
class ObserverSnapshotTests(unittest.TestCase):
    ...
    def test_verifier_rejects_hunter_record_before_registration(self) -> None:
        for steps in (0, 10, 13):
            with self.subTest(steps=steps):
                simulation = Simulation(seed=319)
                simulation.run(steps)
    ...
    def test_verifier_requires_empty_fixed_event_equipment(self) -> None:
        for steps in (10, 13):
            with self.subTest(steps=steps):
                simulation = Simulation(seed=329)
                simulation.run(steps)
                snapshot = observer_snapshot(simulation)
                equipment = snapshot["protagonist"]["equipment"]
    ...
    def test_verifier_rejects_redigested_invalid_equipment(self) -> None:
        ...
        d_rank_locked = observer_snapshot(Simulation(seed=313))
        d_rank_locked["protagonist"]["equipment"]["weapon"] = "Mana-edge Saber"
        _redigest(d_rank_locked)
        with self.assertRaisesRegex(ValueError, "exceeds hunter rank"):
