# Changelog and evaluation history

This file preserves the detailed update record, balance evidence, rejected experiments, checkpoint migrations, and reporting-tool evolution for AWAKENED: ZERO RANK.
...
- Gives strike, barrier, guard, and retreat distinct tactical-card identities with compact numbering, clearer hierarchy, and responsive hover feedback.
- Reworks the encounter heading, plan note, field kit, and footer into one rain-soaked case-file visual language instead of a stack of generic dashboard boxes.
- Keeps desktop and mobile controls readable without changing damage, energy costs, enemy intent, or time advancement.
...
- Kept pixel sprites exclusive to Tokyo exploration while Gate battles use cinematic foreground staging.
- Added responsive combat framing that preserves the enemy, intent telegraph, and battlefield depth.
...
- Rebuilt optional bond presentation around half-body expression sheets for Ren, Aiko, Daichi, Haruto, and Mei.
- Bond dialogue now changes facial acting across narration, conversation, choice, and result beats.
...
- Restores up to 25 HP without exceeding Ren's maximum health.
- Limits treatment to once per campaign day through the persistent action journal.
- Blocks full-health and unaffordable treatment with explicit interface states.
- Added focused regression coverage and increased the browser suite to 149 tests.

## 0.1290.0 - Akihabara field supplies

- Added individually priced bandages, energy drinks, and ward charms to Haruto's market counter.
- Enforces the existing maximum of three bandages, three energy drinks, and one active ward.
- Blocks full-pack and unaffordable purchases before any money is deducted.
- Saves each successful purchase immediately without charging another time slot after city travel.
- Added focused regression coverage and increased the browser suite to 148 tests.

## 0.1280.0 - Tokyo transit economy

- Added a deterministic ¥220 outbound train fare to every playable Tokyo route.
- Shows fare, six-energy cost, and one-slot commitment before Ren travels.
- Blocks unaffordable outbound trips and records the deduction through the standard RPG action pipeline.
- Keeps homeward travel free of cash cost so a low-money save cannot become stranded outside the apartment.
- Added focused regression coverage and increased the browser suite to 147 tests.
...
- Shows the approaching arc deadline, elapsed days, maximum surrendered time slots, income, and energy recovery before saving.
- Warns that skipped meetings, investigations, and training cannot be recovered.
- Added a safe cancel path that leaves the local campaign state untouched and restores the apartment actions.
- Extended pacing coverage to require the confirmation and its accessible presentation.
...
- Made Arc I authenticated proof reduce the deadline's health costs and an Aiko bond of 2 unlock a coordinated evacuation success.
- Added a high-risk live-network read that requires 40% Residual Read mastery and becomes an ordinary lethal Game Over when attempted unprepared.
- Persisted Arc II evidence, survivors, network mapping, partial failure, resources, bonds, time, and location consequences.
...
- Added versioned validation and automatic migration for device-local campaign saves.
- Updated site metadata to accurately describe the separate Observer and RPG modes.
...
- Routed Gate missions to portal dialogue and Guild patrols or Guild-location meetings to the authored Guild exchange instead of flattening both into portal talk.
- Kept routine meetings as the fallback without changing encounter frequency, relationship rewards, or action timing.
...
- Portal evidence is bound to the authored hunter-work unlock across the full pre-registration timeline rather than only two fixed-event snapshots.
- Added pre-Awakening, post-Awakening, and post-registration tamper coverage across all three validation surfaces.

## 0.256.0 - Guild equipment chronology

- Browser, save, and Python observer validation now require an empty hunter loadout through the exact post-registration boundary.
- Equipment evidence is bound to the authored Guild-shop unlock across the full pre-registration timeline rather than only two fixed-event snapshots.
- Added pre-Awakening, post-Awakening, and post-registration tamper coverage across all three validation surfaces.
...
- Day 8 Morning remains pre-processing; day 8 Afternoon and later may carry the deadline result.
- Added exact before/after boundary coverage. Valid timelines, simulator decisions, and schemas are unchanged.
...
- Fractional or out-of-range trust values are rejected, and rendered relationships must have unique names in canonical ascending order.
- Added focused negative-boundary, fractional, out-of-range, reversed-order, and duplicate-identity regression checks.
- Simulator, balance, schemas, RL behavior, observer artifacts, presentation, and read-only capabilities are unchanged.
...
- Removed the three confirmed-unreferenced starter SVG assets while preserving the production favicon, social card, and authenticated demonstration data.
- Added a focused package regression check that requires production observer documentation and prevents the generic starter manual or assets from returning.
- Runtime presentation, simulator, balance, schemas, RL behavior, observer artifacts, and read-only capabilities are unchanged.
...
- Added same-seed continuity metadata and retained seed in changed sections when inputs differ; equal seeds are not presented as proof of an unbranched timeline.
- CLI comparison and equality-gate JSON expose the new metadata so a future public website can avoid animating rewinds or seed switches as normal forward updates.
- Added focused forward, backward, same-clock, and different-seed coverage, raising the deterministic suite to 208 tests; simulation, balance, and RL behavior are unchanged.
...
- Added `--snapshot-output FILE` for `--observer-snapshot` so automation can publish the exact printed payload without shell redirection.
- Existing destinations are never overwritten, a destination that appears during staging aborts publication, and invalid snapshots leave no output artifact.
- Source timeline saves remain byte-identical, source-path provenance remains outside state identity, and published snapshots remain compatible with the read-only verifier.
- Added four focused API and CLI tests for exact payloads, canonical newlines, nested destinations, invalid input cleanup, overwrite refusal, option requirements, and source-save immutability, raising the deterministic suite to 188 tests; simulation balance and RL behavior are unchanged.
...
- Anchor timing is fixed, while each resolution is derived deterministically from Ren's current rank, trusted relationships, and portal discoveries.
- Story events use the existing calendar and event log, survive normal save/load continuation, and preserve the observer-only design without player choices.
- Added focused schedule, ending, readiness-variation, calendar-coexistence, determinism, and continuation coverage, raising the deterministic suite to 158 tests; no RL experiment or balance adjustment was needed.
...
- An impossible current state cannot replace an existing valid timeline or leave an abandoned staging file.
- CLI semantic save failures use the established concise error contract and never print a false success message.
- Added two focused write-safety tests, raising the deterministic regression suite to 148 tests; valid saves and simulation behavior are unchanged.
...
- Refactored loading and verification through one file read, eliminating a check/load race while preserving exact reconstruction.
- Added schema version and integrity status to deterministic verification JSON.
- Added focused legacy-evidence coverage, raising the deterministic regression suite to 144 tests; simulation and RL behavior are unchanged.

## 0.147.0 — Read-only save verification

- Added a `--verify-save` CLI mode that fully loads and integrity-checks a timeline without advancing or rewriting it.
- Emits deterministic JSON metadata for automation and rejects every simulation option.
- Malformed, missing, unsupported, and integrity-failed saves use the existing traceback-free CLI error contract.
- Added three focused tests, raising the deterministic regression suite to 143 tests; simulation and RL behavior are unchanged.

## 0.146.0 — Critical static-analysis gate

- Added a reproducible development extra and checked-in Ruff configuration limited to fatal syntax-family errors and undefined names.
- Runs the static gate inside the existing Python 3.14 core job, adding coverage without another CI machine.
- Updated contributor and README verification commands to match CI.
- The full 140-test count, simulation behavior, persistence schemas, and RL verdicts are unchanged.

## 0.145.0 — Current Python compatibility matrix

- Expanded bounded core CI from Python 3.11–3.12 to every stable minor from 3.11 through 3.14.
- Moved clean wheel and source-distribution validation to Python 3.14 while retaining the dedicated Python 3.12 Gymnasium lane.
- Added explicit package classifiers for all four tested Python minors.
- Test count, simulation behavior, persistence schemas, and RL verdicts are unchanged.

## 0.144.0 — Unambiguous CLI run inputs

- Explicit seed selection and saved-timeline loading are now mutually exclusive, preventing a supplied seed from being silently ignored.
- Nonpositive day horizons now use the same concise argparse error contract as other invalid CLI inputs.
- Added focused exit-code, output, and traceback-absence coverage, raising the deterministic regression suite to 140 tests.
- Valid commands, simulation behavior, persistence schemas, and RL verdicts are unchanged.
...
- Declared the README, project keywords, Python classifiers, repository URL, and changelog URL in `pyproject.toml`.
- Added deterministic CLI coverage and raised the verified suite to 132 tests.

## 0.136.0 — Documentation restructure

- Replaced the research-log-style README with a concise product overview and operator guide.
- Moved the complete historical evaluation and balance record here without discarding prior evidence.
- Kept the current policy verdict, roadmap, and experiment workflow visible from the README.

## Detailed evaluation and balance history (0.24–0.135)
...
Update 0.84 — Authenticated Realized Reward Evidence records cumulative unshaped environment reward for every training state/action pair and advances Q-table checkpoints to schema 18; schema-17 and older checkpoints load with reward evidence explicitly unavailable. Reward totals reconcile exactly with episode returns and the new table is covered by SHA-256 tamper rejection. In the balanced-grid replay, 49 preparation selections accumulated −52.620 immediate reward (−1.074 average); all six repeated safety groups were negative, ranging from −0.960 to −0.520 across 29 visits. This does not contradict the paired evidence that preparation improves later missions—it confirms that immediate cost and delayed benefit must be attributed separately. No reward or policy change was made, and the verdict remains **baseline remains better**.

Update 0.85 — Authenticated Discounted Return Evidence records cumulative unshaped Monte Carlo return from every training decision to its fixed episode horizon and advances checkpoints to schema 19; schema-18 and older checkpoints load with return evidence explicitly unavailable. One-step returns reconcile exactly with realized rewards, repeated training is deterministic, and the table is directly covered by tamper rejection. In the balanced-grid replay, the 49 preparation visits averaged +7.662 discounted return and 31 were positive; all six repeated safety groups had positive averages from +1.287 to +12.423, but every group still contained mixed-sign member states. Delayed preparation value is **promising but inconclusive** because episode returns include unrelated later events. No reward or policy change was made, and the overall verdict remains **baseline remains better**.
...
Update 0.88 — Authenticated Plan Contexts align every independent preparation-plan lifecycle with its training condition and fixed horizon, advancing checkpoints to schema 22 while schema-21 policies retain lifecycle results with context explicitly unavailable. In the balanced-grid replay, 20-step cells produced only one consumed plan in total and injury-recovery plus compound-crisis produced no plan; 60-step cells produced 18 consumed plans. Long-horizon consumed returns were positive on average in standard (+7.702), financial pressure (+9.250), Gate crisis (+15.743), and compound crisis (+11.619), while the single injury-recovery consumption was negative (−11.405). Preparation value remains **promising but inconclusive** because evidence is long-horizon concentrated and sparse within cells. No reward or policy change was made, and the overall verdict remains **baseline remains better**.
...
Update 0.91 — Replicated Deficit Attribution replays diagnostics for the same 60 RL-versus-utility episode pairs. RL encountered unseen states on 2,127/2,193 decision steps (97.0%); average component gaps were −13.543 survival, −12.007 progress, −0.250 social, and +0.997 stability. RL spent 393 steps at critical energy versus utility's 30 and completed 31 missions versus 99, although prepared completions favored RL 15–9. Gate and preparation each had 1,906 unseen versus 182 seen opportunities; fallback selected Gate 17 times and preparation 21 times, while preparation was greedy in 0/182 seen opportunities. Sparse transfer, fallback behavior, energy burden, and missed mission progress—not preparation quality—dominate the deficit. No behavior changed; checkpoint schema remains 22 and the verdict is **baseline remains better**.

Update 0.92 — Critical-Energy Control Attribution replays the same three frozen policies and splits all 381 critical-energy decisions by controller. Unseen heuristic fallback controlled 380, choosing Rest 319 times and Eat 61 times; the remaining decision was an existing preventive Rest override. Seen Q-table choices controlled zero critical-energy decisions. A new critical-energy safeguard would therefore duplicate existing recovery behavior rather than fix the burden; the unresolved cause is how episodes enter or remain in critical energy. No safeguard was added, checkpoint schema remains 22, and the verdict is **baseline remains better**.
...
Update 0.100 — Seen-State Action and Outcome Attribution records each resolved seen-state learned action, its non-mutating utility alternative, realized reward components, and mission completion, then aggregates action pairs in diagnostic reports. On the same three-policy fresh-seed audit, 448/532 disagreements (84.2%) were learned Eat or Rest choices; utility alternatives most often favored Part-time work (182), Guild patrol (140), or Seek treatment (92). Disagreement steps averaged +1.954 realized reward versus +3.418 on agreement steps, with the largest observed difference in stability, while learned Gate choices produced 38 mission completions among disagreement steps. These are context-selected realized outcomes, not causal counterfactual rewards, so the evidence is mixed and no action priority changes. Checkpoint schema remains 24 and the verdict remains **baseline remains better**.
...
Update 0.107 — Downstream Critical-Entry Controller Traces record every actual >25→≤25 energy transition with controller, action, pre/post energy, and up to three preceding actions. Across selective-control confirmation groups 1001–1020 and 1101–1120, 27/31 entries were controlled by unseen utility fallback, three by the Q-table, and only one directly by selective recovery. Guild patrol caused 24 entries and Gate mission seven; 21 followed Seek treatment → Guild patrol, while four followed Prepare portal → Gate mission. Pre-entry energy clustered at 38. The safety burden is therefore primarily a downstream utility-fallback trajectory effect, not a direct selective override or learned recovery choice. No new safeguard is justified, behavior and checkpoint schema 25 remain unchanged, the selective control stays default-off, and the overall verdict remains **baseline remains better**.
...
Update 0.113 — Authenticated Condition–Horizon Feature Slices require a single condition and horizon whose episode labels exactly match the training config, then report sparse categorical coverage from its visit table. A bounded three-replication 5×2 slice matrix found that 20-step runs kept money in category 0 for four of five conditions and rank points in category 0 for four, while every 60-step condition reached a second category for both features. Condition-specific constants aligned with intentional crisis initialization, and no money slice reached categories 2–3. The evidence supports retaining both the encoder and balanced 20/60 schedule; behavior and checkpoint schema 25 remain unchanged, and the honest verdict remains **baseline remains better**.
...
Update 0.124 — Authenticated Experiment Catalogs add a canonical schema-1 discovery layer over scenario-suite and similarity-audit reports. Catalog entries derive rather than accept report digests and metadata, use portable relative JSON filenames, sort deterministically, and expose report type, training seeds, conditions, horizons, and status. Duplicate labels, filenames, or digests; absolute, parent-traversing, or platform-specific paths; malformed metadata; unsupported versions; and tampered catalogs are rejected, including re-hashed invalid inputs. This gives a future developer dashboard one safe index without embedding machine-specific paths or running experiments. No RL experiment, policy behavior, checkpoint schema, or verdict changed; single-policy RL remains **baseline remains better** and the corrected return-evidence ensemble remains **inconclusive**.
...
Update 0.127 — Verified Bundle Inspection adds a compact immutable summary of a published bundle after full catalog-to-file verification: catalog digest, report count and type counts, unique training seeds, conditions, horizons, and status counts. The new `--inspect-experiment-bundle DIR` CLI mode emits deterministic JSON only, rejects explicit simulation options, and exits cleanly on missing or tampered data. Existing simulation defaults remain seven days and seed 42 when those options are omitted. This provides a scriptable ingestion point for future dashboard work without running training or simulation. No RL experiment, policy behavior, checkpoint schema, or verdict changed; single-policy RL remains **baseline remains better** and the corrected return-evidence ensemble remains **inconclusive**.

Update 0.128 — Verified Bundle Comparison fully authenticates two published bundles before producing canonical JSON differences for added, removed, changed, and unchanged report files; per-file status and report-type transitions; and aggregate training-seed, condition, and horizon changes. The new `--compare-experiment-bundles LEFT RIGHT` mode is mutually exclusive with inspection and rejects simulation options. This update ran only tiny two-episode fixtures, not a new RL experiment; policy behavior and checkpoint schema 26 are unchanged, single-policy RL remains **baseline remains better**, and the corrected return-evidence ensemble remains **inconclusive**.
...
Update 0.131 — Complete Bundle Delta Records extend canonical comparisons with full authenticated metadata for every added and removed report, alongside the field-level before/after records for changed reports. Consumers can now render the complete verified delta from one JSON document without reopening either catalog. Identical comparisons emit empty added, removed, and changed record collections; existing summary fields and CI behavior remain compatible. No additional RL experiment, policy behavior, checkpoint schema, or verdict changed; single-policy RL remains **baseline remains better** and the corrected return-evidence ensemble remains **inconclusive**.
...
| 0.144 | Unambiguous timeline-origin and horizon validation |
| 0.145 | Python 3.11–3.14 compatibility matrix |
