...
const BEATS = [
  {
    speaker: "DISPATCH",
    line: "Seven synchronized breaches. North and east evacuation corridors are collapsing together.",
  },
  {
    speaker: "AIKO",
    line: "I can keep the civilians moving, but only if you trust my route and hold the pulse away from them.",
  },
  {
    speaker: "REN",
    line: "The fractures share one repeating vector. Residual Read can find it—but reading the live network may tear him apart.",
  },
] as const;
...
export default function ArcTwoDeadlinePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (pendingStoryRoute(save) !== "/game/deadline/arc-two")
  ...
  const resolve = useCallback(
    (choice: "route" | "network" | "vector" | "causal") => {
      ...
      if (choice === "causal" && rpg.timeline === 3 && causal >= 55) {
        ...
        events.add("arc-ii-evidence");
        events.add("adachi-civilians-survived");
        events.add("timeline-ii-seven-route-rescue");
        bonds["Aiko Sato"] = Math.min(10, (bonds["Aiko Sato"] ?? 0) + 2);
        action = "Vector Stepped across all seven evacuation routes";
        result = {
          success: true,
          title: "Seven routes move as one.",
          copy: "Ren follows the remembered pulse and appears at every collapsing junction before it closes. Aiko evacuates the whole district and preserves the network map.",
        };
      } else if (choice === "route") {
        health -= priorProof ? 10 : 15;
        energy -= 18;
        if (trusted) {
          events.add("arc-ii-evidence");
          events.add("adachi-civilians-survived");
          bonds["Aiko Sato"] = Math.min(10, (bonds["Aiko Sato"] ?? 0) + 2);
          action = "Held the pulse for Aiko's evacuation";
          result = {
            success: true,
            title: "The corridors hold.",
            copy: "Aiko's route clears the district while Ren diverts the synchronized pulse. Survivor records authenticate the second pattern.",
          };
        } else {
          events.add("arc-ii-deadline-failed");
          action = "Held one corridor without a coordinator";
          result = {
            success: false,
            title: "One route survives.",
            copy: "Ren holds the north corridor alone. The east route collapses, and the surviving records are too incomplete to prove coordination.",
          };
        }
      } else {
        energy -= 26;
        if (mastery >= 40) {
          health -= priorProof ? 22 : 30;
          events.add("arc-ii-evidence");
          events.add("synchronized-network-mapped");
          action = "Read and split the synchronized Gate network";
          result = {
            success: true,
            title: "Seven pulses become one map.",
            copy: "Ren reads the shared vector and forces the breaches out of phase. The district survives with a complete causal map.",
          };
        } else {
          health = 0;
          events.add("arc-ii-deadline-failed");
          action = "Was consumed by the synchronized Gate network";
          result = {
            success: false,
            title: "The network reads him back.",
            copy: "Residual Read cannot contain seven live breaches below 40% mastery. Ordinary death ends this run.",
          };
        }
      }
      setRpg(
        takeRpgAction(
          ...
          action,
          {
            health,
            energy,
            location: "Adachi Breach Corridor",
            bonds,
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      ...
      if (beat < BEATS.length && (event.key === "Enter" || event.key === " ")) {
        ...
        resolve("route");
      else if (beat >= BEATS.length && event.code === "Digit2")
        resolve("network");
      else if (
        beat >= BEATS.length &&
        event.code === "Digit3" &&
  ...
  if (failed)
    return (
      <main className="game-loading">
        ...
        <h1>The Arc II record could not be verified.</h1>
  ...
  if (!rpg)
    return (
      <main className="game-loading" aria-busy="true">
        <p>DAY 120 / DISTRICT BREACH</p>
        <h1>Opening the evacuation corridor…</h1>
      </main>
    );
  const trusted = (rpg.bonds["Aiko Sato"] ?? 0) >= 2,
  ...
  return (
    <main
      className="deadline-cutscene arc-two-deadline"
      aria-label="Arc II Adachi Countdown deadline"
    >
      <Image
        className="deadline-bg"
        src="/game/cutscenes/adachi-day120-breach-v1.png"
        alt="District-wide Gate breach across an Adachi evacuation corridor"
        fill
        sizes="100vw"
        priority
      />
      ...
      <Image
        ...
        alt="Ren Takahashi facing the synchronized breach"
        width={1024}
        height={1536}
        priority
      />
      ...
      <header className="deadline-caption">
        <span>
          ARC II DEADLINE / TIMELINE {rpg.timeline} / DAY {rpg.day}
        </span>
        <b>THE ADACHI COUNTDOWN</b>
        <small>
          {priorProof ? "ARC I PROOF RETAINED" : "NO PRIOR PROOF"} · AIKO BOND{" "}
          {rpg.bonds["Aiko Sato"] ?? 0} · RR {mastery}%
        </small>
      </header>
      ...
      <section className="deadline-panel" aria-live="polite">
        {outcome ? (
          ...
          >
            <small>
              {rpg.status === "game-over" ? "GAME OVER" : "ARC II RESOLVED"}
            </small>
            <h1>{outcome.title}</h1>
            <p>{outcome.copy}</p>
            <dl>
              ...
              <div>
                <dt>NEXT</dt>
                <dd>{rpg.status === "game-over" ? "RETRY" : "ARC III"}</dd>
              </div>
            </dl>
            <Link href="/game">
          ...
          <div className="deadline-choice">
            ...
            <h1>
              Seven breaches. {rpg.timeline >= 2 ? "Three" : "Two"} ways to
              answer.
            </h1>
            <p>
              Trust and mastery were built before tonight. Ren cannot create
              either at the deadline.
            </p>
            <div>
              <button onClick={() => resolve("route")}>
                <b>1</b>
                <span>HOLD AIKO'S EVACUATION ROUTE</span>
                <small>
                  {trusted
                    ? "BOND READY · EVIDENCE + SURVIVORS"
                    : "BOND BELOW 2 · PARTIAL FAILURE"}
                </small>
              </button>
              <button onClick={() => resolve("network")}>
                <b>2</b>
                <span>READ THE LIVE BREACH NETWORK</span>
                <small>
                  {mastery >= 40
                    ? "40% MASTERY READY · SEVERE COST"
                    : "40% MASTERY REQUIRED · LETHAL"}
                </small>
              </button>
              {rpg.timeline >= 2 && (
                <button
