...
import { bondAvailability, loadRpgState, takeRpgAction, type RpgState } from "../game-state";
...
export default function CityRoutePage() {
  ...
  function reset() { setInspected([]); setActive(null); setChoice(null); setBondResult(null); }
  ...
  function spendTime(name: string) {
    ...
    const bonds = { ...rpg.bonds, [name]: Math.min(10, (rpg.bonds[name] ?? 0) + 1) };
    setRpg(takeRpgAction(rpg, `Spent time with ${name}`, { energy: rpg.energy - 4, bonds }));
  ...
  const routes = snapshot.whereabouts.map((whereabout) => ({ ...whereabout, relationship: snapshot.relationships.find((item) => item.name === whereabout.name), availability: bondAvailability(whereabout.name, rpg) }));
  ...
  return <main id="chronicle" className="city-shell">
    ...
    <section className="city-intro"><small>DAY {rpg.day} / {rpg.slot} / {rpg.location}</small><h1>Choose where Ren<br />goes next.</h1><p>Explore the districts, inspect two signals, then travel. The trip spends one RPG time slot.</p>{rpg.timeline >= 2 && <Link className="timeline-route" href="/game/residual-relay"><b>SECOND-TIMELINE ROUTE</b><span>HANEDA RESIDUAL RELAY</span><small>VECTOR STEP · BUSAN SIGNAL · ANCHOR</small></Link>}</section>
    ...
    <section className="route-board" aria-label="Tokyo route board">
      <div className={`route-map city-diorama district-${district.id}`}><Image className="tokyo-map-bg" src={district.image} alt={`Pixel-art ${district.label.toLowerCase()}`} fill sizes="(max-width: 800px) 90vw, 65vw" priority /><div className="map-atmosphere" aria-hidden="true" /><span className="district-stamp">{district.label}<b>LOCAL MAP</b></span><span className="map-origin"><Image src="/game/characters/ren.png" alt="Pixel sprite of Ren Takahashi" width={72} height={72} /><small>REN</small><b>VIEWING {district.label}</b></span>{visibleRoutes.map((route, index) => <button key={route.name} className={`route-node node-${index + 1} ${inspected.includes(route.location) ? "inspected" : ""} ${route.availability.available ? "contact-available" : "contact-away"}`} onClick={() => inspect(route.location)} aria-pressed={active === route.location}><span className="route-art"><Image className="landmark-sprite" src={LANDMARKS[route.location]} alt="" width={120} height={120} /><Image className="contact-sprite" src={SPRITES[route.name]} alt="" width={48} height={48} /></span><span className="route-label"><small>{route.name}</small><b>{route.location}</b><em>{route.availability.status}</em></span></button>)}</div>
      <aside className="route-dossier" aria-live="polite"><div className="game-progress"><span>SIGNALS CHECKED</span><b>{inspected.length} / {routes.length}</b></div>{!activeRoute && <div className="game-copy"><small>CITY MAP</small><h2>Choose Ren's destination.</h2><p>Contacts follow local schedules. Story-critical meetings still trigger automatically and cannot be missed.</p></div>}{activeRoute && <div className="game-copy character-dossier"><Image src={SPRITES[activeRoute.name]} alt={`Pixel sprite of ${activeRoute.name}`} width={80} height={80} /><small>{activeRoute.relationship?.role ?? "KNOWN CONTACT"}</small><h2>{activeRoute.name}</h2><p>{activeRoute.location}. {activeRoute.availability.status}. {activeRoute.availability.schedule}</p></div>}<div className="route-options"><small>TRAVEL AS REN</small>{routes.map((route) => <button key={route.name} disabled={!ready || choice !== null} onClick={() => chooseRoute(route.name, route.location)}><span>{route.location}</span><small>TRAVEL · {route.availability.status}</small></button>)}{!ready && <p>Inspect {2 - inspected.length} more signal{2 - inspected.length === 1 ? "" : "s"} first.</p>}</div></aside>
    </section>
    {choice && choiceAvailability && <section className="route-result" aria-labelledby="route-result-title"><div className="route-encounter" aria-hidden="true"><Image src="/game/characters/ren.png" alt="" width={80} height={80} /><i /><Image className={choiceAvailability.available ? "" : "contact-absent"} src={SPRITES[choice.name]} alt="" width={80} height={80} /></div><small>TRAVEL COMPLETE / TIME ADVANCED</small><h2 id="route-result-title">{choice.location}</h2><blockquote>{bondResult ?? routeResponse(snapshot, choice.name, choice.location)}</blockquote><p>{choiceAvailability.status}. {choiceAvailability.schedule} RPG clock: Day {rpg.day}, {rpg.slot}. Energy {rpg.energy}.</p><nav>{choiceAvailability.available && !bondResult && <button className="primary" onClick={() => spendTime(choice.name)}>SPEND TIME WITH {choice.name.toUpperCase()}</button>}<Link href="/game/caseboard">CONTINUE TO GATE FIELD</Link><button onClick={reset}>EXPLORE ANOTHER ROUTE</button></nav></section>}
