...
export default function FieldPage() {
  ...
  return <main id="chronicle" className="field-shell">
    ...
    <section className="field-stage" aria-label="Gate battle">
      <div className={`field-arena enemy-${encounter.className} intent-${intent.id} city-${rpg.slot.toLowerCase().replace(" ", "-")} weather-${atmosphere.weather.toLowerCase()} snow-depth-${atmosphere.snowDepth}`}><Image src={mapImage} alt={`Pixel-art Gate exclusion zone during ${atmosphere.label}`} fill sizes="(max-width: 800px) 100vw, 70vw" priority /><div className="field-shade" />{(atmosphere.weather === "Rain" || atmosphere.weather === "Snow") && <div className="city-weather field-weather" aria-hidden="true"><i /><i /><i /></div>}<span className="field-ren"><Image src="/game/characters/ren.png" alt="Pixel sprite of Ren Takahashi" width={96} height={96} /><b>REN</b></span><span className="field-enemy" aria-label={encounter.enemy}><i /><i /><i /><b>{encounter.enemy}</b></span><div className="enemy-intent"><small>NEXT ATTACK / {intent.label}</small><b>{intent.damage} DAMAGE</b><p>{intent.cue}</p></div></div>
