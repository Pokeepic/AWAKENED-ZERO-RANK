...
export default function FieldPage() {
  ...
  return <main id="chronicle" className="field-shell">
    ...
    <section className="field-stage" aria-label="Gate battle">
      <div className={`field-arena portal-interior enemy-${encounter.className} intent-${intent.id}`}><Image src={encounter.background} alt={`Illustrated interior of ${caseFile?.portal_name ?? "the verified Gate"}`} fill sizes="(max-width: 800px) 100vw, 70vw" priority /><div className="field-shade" /><span className="field-ren field-ren-battle"><Image src="/game/characters/ren-battle-back-v1.png" alt="Ren Takahashi viewed from behind in a defensive combat stance" width={1024} height={1536} priority /><b>REN</b></span><span className="field-enemy" aria-label={encounter.enemy}><i /><i /><i /><b>{encounter.enemy}</b></span><div className="enemy-intent"><small>NEXT ATTACK / {intent.label}</small><b>{intent.damage} DAMAGE</b><p>{intent.cue}</p></div></div>
