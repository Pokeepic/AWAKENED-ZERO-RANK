...
type CombatFeedback = { dealt: number; incoming: number; restored: number; blocked: boolean; finisher: boolean };
...
export default function FieldPage() {
  ...
  const [feedback, setFeedback] = useState<CombatFeedback>({ dealt: 0, incoming: 0, restored: 0, blocked: false, finisher: false });
  ...
  const act = useCallback((id: MoveId) => {
    ...
    setFeedback({ dealt, incoming, restored: 0, blocked: enemyHp > 0 && incoming === 0, finisher: enemyHp === 0 });
  ...
  const applyFieldItem = useCallback((item: "bandage" | "energyDrink") => {
    ...
    setFeedback({ dealt: 0, incoming, restored: item === "bandage" ? restoredHealth - rpg.health : energy - rpg.energy, blocked: incoming === 0, finisher: false });
  ...
  return <main id="chronicle" className="field-shell">
    ...
    <section className="field-stage" aria-label="Gate battle">
      <div key={`${caseName}-${attackMotion.count}`} className={`field-arena portal-interior enemy-${encounter.className} intent-${intent.id} move-${attackMotion.move} ${turnLocked ? "battle-attacking" : ""} ${turnLocked && attackMotion.counter ? "enemy-countering" : ""} ${turnLocked && feedback.finisher ? "finishing-blow" : ""}`}><Image src={encounter.background} alt={`Illustrated interior of ${caseFile?.portal_name ?? "the verified Gate"}`} fill sizes="(max-width: 800px) 100vw, 70vw" priority /><div className="field-shade" /><span className="field-ren field-ren-battle"><Image className="ren-battle-idle" src="/game/characters/ren-battle-back-v2.png" alt="Ren Takahashi viewed from behind in a defensive combat stance" width={1024} height={1536} priority /><Image className="ren-battle-strike" src="/game/characters/ren-battle-strike-v1.png" alt="" aria-hidden="true" width={1024} height={1536} priority /><i className="barrier-effect" aria-hidden="true" /><i className="read-effect" aria-hidden="true" /><i className="counter-impact" aria-hidden="true" /><b>REN</b></span><span className="field-enemy" aria-label={encounter.enemy}><i /><i /><i /><b>{encounter.enemy}</b></span>{turnLocked && <div className="combat-feedback" aria-hidden="true">{feedback.dealt > 0 && <strong className="damage-enemy">-{feedback.dealt}</strong>}{feedback.incoming > 0 && <strong className="damage-ren">-{feedback.incoming}</strong>}{feedback.blocked && <strong className="blocked-hit">BLOCKED</strong>}{feedback.restored > 0 && <strong className="restored-stat">+{feedback.restored}</strong>}{feedback.finisher && <strong className="finisher-label">BREAK</strong>}</div>}<div className="enemy-intent"><small>NEXT ATTACK / {intent.label}</small><b>{intent.damage} DAMAGE</b><p>{intent.cue}</p></div></div>
