...
export default function EveningPage() {
  ...
  const answer = useCallback((response: (typeof RESPONSES)[number]) => {
    ...
    setChoice(response);
    setRpg(takeRpgAction(rpg, `Evening with Aiko: ${response.id}`, { energy: Math.min(100, rpg.energy + response.energy), location: "Ren's Apartment", bonds, completedEvents }, remainingDaySlots(rpg)));
  }, [choice, rpg]);

  const eventCompleted = rpg?.completedEvents.includes(EVENT_ID) ?? false;
  const eventEligible = rpg ? pendingStoryRoute(rpg) === "/game/evening" : false;
  useEffect(() => {
