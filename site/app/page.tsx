...
export default function Home() {
  ...
  return <main id="chronicle" tabIndex={-1} className={updated ? "chronicle-updated" : undefined}>
    ...
    <div className="grid">
      ...
      <section className="atlas"><h2 className="section-label">PORTAL ATLAS <span>{discovered.length}/{PORTAL_PROFILE_CATALOG.length} DOCUMENTED</span></h2><details className="reference-shelf"><summary>INSPECT COMPLETE PORTAL CATALOG <span>{PORTAL_PROFILE_CATALOG.length} PROFILES</span></summary><div className="atlas-grid">{PORTAL_PROFILE_CATALOG.map((portal) => { const known = discovered.includes(portal.name); return <article key={portal.name} className={known ? "known" : "unknown"}><small>{portal.environment.toUpperCase()} / {known ? "DOCUMENTED" : "UNDISCOVERED"}</small><b>{portal.name}</b><span>{known ? `HAZARD / ${portal.hazard}` : "HAZARD / CLASSIFIED"}</span><p>{known ? `VERIFIED EFFECT / ${portal.aftermath}` : "EFFECT / CLASSIFIED"}</p></article>; })}</div></details></section>
