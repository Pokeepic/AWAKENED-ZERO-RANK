...
| Automated tests | 339 Python + 82 browser |
...
The production observer now covers time, weather, Ren's condition and intent, explained decisions, finances, inventory, hunter progression, story history, relationships, current whereabouts, an authenticated current-scene dossier, a collapsible thirteen-place Tokyo atlas, portal investigations, memories, and artifact identity. The scene and atlas form a read-only foundation for the future point-and-click presentation direction; lightweight sprite animation remains optional rather than a requirement for the completed observer.
