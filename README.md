...
Six fixed story anchors arrive at roughly six-month intervals from day 183 through day 1,095. They cannot be avoided or moved, but each resolution reflects Ren's accumulated hunter rank, trusted relationships, and discovered portals. The final anchor closes the three-year chronicle with an ending shaped by that history. Every resolved anchor records an `isolated`, `resilient`, or `prepared` outcome in the saved world state, and the observer summary reports current arc progress plus the latest result.
